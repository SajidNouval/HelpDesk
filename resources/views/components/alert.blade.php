@props(['type' => 'info', 'title' => null, 'dismissible' => false, 'inline' => false])
...
$containerClass = $inline ? '' : 'rounded-xl border shadow-lg px-4 py-3 flex items-start gap-3';
@endphp

@if($inline)
<div {{ $attributes->merge(['class' => 'rounded-xl border shadow-lg px-4 py-3 flex items-start gap-3 ' . $style['bg'] . ' ' . $style['border'] . ' ' . $style['text']]) }}>
@else
<div {{ $attributes->merge(['class' => 'rounded-xl border shadow-lg px-4 py-3 flex items-start gap-3 ' . $style['bg'] . ' ' . $style['border'] . ' ' . $style['text']]) }}>
@endif
    ...
    <div class="flex-1 min-w-0">
        ...
        @endif
        <p class="text-sm text-gray-600 {{ $title ? 'mt-1' : '' }}">{{ $slot }}</p>
    </div>
    @if($dismissible)
        <button type="button" class="flex-shrink-0 ml-auto -mx-1.5 -my-1.5 rounded-lg focus:ring-2 p-1.5 inline-flex items-center justify-center h-8 w-8 {{ $style['text'] }} hover:bg-opacity-20 hover:bg-black focus:outline-none" data-dismiss-alert>
