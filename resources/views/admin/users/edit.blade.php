                ...
                <div class="grid gap-8">
                    <div class="grid gap-6 lg:grid-cols-[320px_1fr]">
                        <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
                            <h3 class="text-lg font-semibold text-gray-900">Informasi Staf</h3>
                            <p class="mt-2 text-sm text-gray-600">Detail ringkas tentang profil staf dan kontribusi terhadap knowledge base.</p>

                            <dl class="mt-6 space-y-5 text-sm text-gray-600">
                                <div>
                                    <dt class="text-gray-500">Bergabung sejak</dt>
                                    <dd class="mt-1 font-semibold text-gray-900">{{ $user->created_at->format('d M Y') }}</dd>
                                </div>
                                <div>
                                    <dt class="text-gray-500">Total Artikel</dt>
                                    <dd class="mt-1 font-semibold text-gray-900">{{ $user->articles_count }}</dd>
                                </div>
                                <div>
                                    <dt class="text-gray-500">Role saat ini</dt>
                                    <dd class="mt-1 font-semibold text-gray-900">{{ ucfirst($user->role) }}</dd>
                                </div>
                                <div>
                                    <dt class="text-gray-500">Status akun</dt>
                                    <dd class="mt-1">
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold {{ $user->status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-700' }}">
                            ...
                            <form id="staff-form" action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-6">
                                ...
                                <div class="grid gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select id="role" name="role" required
                                                class="w-full px-4 py-3 border border-gray-300 rounded-2xl text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition">
                                ...
                                <div class="flex justify-end pt-4 border-t border-gray-200">
                                    <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                                        <a href="{{ route('admin.users.index') }}" class="inline-flex items-center justify-center rounded-2xl border border-gray-300 text-gray-700 bg-white px-4 py-2 text-sm font-medium hover:bg-gray-50 transition">Batal</a>
                                        <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-red-600 hover:bg-red-700 text-white text-sm font-medium px-4 py-2 transition">Simpan Perubahan</button>
                                    </div>
                                </div>
                            </form>
                        </div>
