    ...
    <div class="bg-gray-100 py-6 border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4">
            ...
            <p class="mt-1 text-sm text-gray-500">
                Tinjau, setujui, atau tolak artikel bantuan yang dibuat oleh Staf.
            </p>
            ...
            <div class="col-span-12 md:col-span-9">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">Daftar artikel yang memerlukan moderasi atau yang sudah diterbitkan.</p>
                    </div>
                </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                @forelse($articles as $article)
                                    <tr data-article-id="{{ $article->id }}" class="hover:bg-gray-50 transition">
                                        <td class="px-4 py-3">
                                            <x-truncate-text :value="$article->title" class="text-sm font-medium text-gray-900 block" />
                                        </td>
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('admin.articles.show', $article) }}" class="inline-flex items-center h-8 px-3 rounded-xl border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 text-xs font-medium transition">
                                                    Lihat
                                                </a>
                                                ...
                                                @if($article->publish_status === 'pending' || $article->publish_status === 'rejected')
                                                    <form action="{{ route('admin.articles.approve', $article) }}" method="POST" class="inline">
                                                        @csrf
                                                        <button type="submit" class="inline-flex items-center h-8 px-3 rounded-xl bg-red-600 hover:bg-red-700 text-white text-xs font-medium transition" data-ajax data-confirm="Apakah Anda yakin ingin menyetujui artikel ini?">
                                                            Setujui
                                                        </button>
                                                    </form>
                                                @endif
                                                ...
                                                @if($article->publish_status === 'pending' || $article->publish_status === 'approved')
                                                    <button type="button" onclick="openRejectModal('{{ route('admin.articles.reject', $article) }}')" class="inline-flex items-center h-8 px-3 rounded-xl border border-red-600 text-red-600 hover:bg-red-50 text-xs font-medium transition">
                                                        Tolak
                                                    </button>
                                                @endif
                                                ...
                                                <form action="{{ route('admin.articles.toggle-hide', $article) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" class="inline-flex items-center h-8 px-3 rounded-xl border {{ $article->is_hidden ? 'bg-yellow-50 text-yellow-700 border-yellow-200 hover:bg-yellow-100' : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100' }} text-xs font-medium transition" data-ajax>
