                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm">
                    <div class="p-6">
                        ...
                        <div class="pt-4 border-t border-gray-200 flex flex-wrap items-center justify-end gap-3">
                            @if($article->publish_status !== 'approved')
                                <form id="approveForm" method="POST" action="{{ route('admin.articles.approve', $article) }}" data-confirm="Setujui artikel ini?" class="inline">
                                    @csrf
                            ...
                            @if($article->publish_status === 'approved')
                                <form action="{{ route('admin.articles.toggle-hide', $article) }}" method="POST" class="inline-block">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center h-10 px-5 rounded-xl border {{ $article->is_hidden ? 'bg-yellow-50 text-yellow-700 border-yellow-200 hover:bg-yellow-100' : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100' }} text-sm font-medium transition gap-2" title="{{ $article->is_hidden ? 'Tampilkan Arsip' : 'Arsipkan' }}" data-ajax>
                                        @if($article->is_hidden)
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
